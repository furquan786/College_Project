...
class AdminStudentProfileScreen extends StatefulWidget {
  ...
  AdminStudentProfileScreen(
      {required this.id,
      required this.putStudentid,
      required this.putStudenttoken});
  @override
...
class _AdminStudentProfileScreenState extends State<AdminStudentProfileScreen> {
  ...
  @override
  void initState() {
    getteacher(widget.id, widget.putStudentid, widget.putStudenttoken);
    print("${widget.putStudentid}");
    print("${widget.id}");
    super.initState();
  }

  Future<void> deletestudent(var teachertoken, var id) async {
    final response = await http.delete(
      Uri.parse(
          'https://aitsudaipur.herokuapp.com/api/admin/delete/student/$id'),
  ...
  Future<void> getteacher(String id, var studentid, var studenttoken) async {
    final response = await http.get(
      Uri.parse(
          'https://aitsudaipur.herokuapp.com/api/admin/get/student/$id/$studentid'),
      headers: {
    ...
    setState(() {
      if (response.statusCode == 200) {
        ...
        if (data != null) {
          ...
          department = data['student']['branch_name'];
          mno = data['student']['mno'];
        } else {
          print("error");
        }
      } else {
